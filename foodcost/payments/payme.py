...
def build_payme_payment_url(order) -> str:
    ...
    # back after payment. We pass the success URL with order_number baked
    # in, so the frontend success page knows WHICH order to show. Payme
    # doesn't add merchant params to return URLs automatically.
    #
    # Note on `;` characters: the c= value lives inside a `;`-separated
    # string and is then base64 encoded as a whole. So a `;` inside the URL
    # itself would break the parameter parsing on Payme's side. Our return
    # URLs never contain `;` (just `?` and `&`), but be aware.
    return_url_base = (getattr(settings, "PAYME_SUCCESS_URL", "") or "").strip()
    return_url = _append_order_number(return_url_base, order) if return_url_base else ""
...
def _append_order_number(base_url: str, order) -> str:
    """
    Append ?order_number=... (or &order_number=...) to a return URL.
    Idempotent — if the URL already carries order_number, we don't add
    another. Falls back to numeric id when public_order_number is empty.
    """
