...
def techcards_page(request, country_slug):
    ...
    # Заготовки — своя техкарта: состав из продуктов и под-заготовок, вес и время.
    preparations = (
        Preparation.objects.filter(country=country)
        .prefetch_related("items__product", "subitems__sub_preparation")
        .order_by("name")
    )
    for p in preparations:
        ...
        prep_subs = [
            ...
            for it in p.subitems.all()
        ]
        if not prep_products and not prep_subs:
            continue  # пустые заготовки не показываем
        rows.append({
            "is_prep": True,
            "prep": p,
            "category": "Заготовки",
            "product_items": prep_products,
            "sub_items": prep_subs,
        })
